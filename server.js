...
const INSTRUMENT_URL = String(process.env.INSTRUMENT_URL || 'https://images.dhan.co/api-data/api-scrip-master-detailed.csv').trim();
...
function parseCSV(text){
  ...
  for(let i=1;i<lines.length;i++){
    const vals=parseCSVLine(lines[i]); const o={};
    headers.forEach((h,j)=>o[h]=vals[j] ?? ''); rows.push(o);
  }
  return rows;
}
...
async function loadInstruments(force=false){
  ...
  instruments=rows.map(x=>({
    exchange: x.EXCH_ID || x.SEM_EXM_EXCH_ID || '',
    segment: x.SEGMENT || x.SEM_SEGMENT || '',
    exchangeSegment: x.EXCH_ID && x.SEGMENT ? `${x.EXCH_ID}_${x.SEGMENT}` : (x.SEM_EXM_EXCH_ID && x.SEM_SEGMENT ? `${x.SEM_EXM_EXCH_ID}_${x.SEM_SEGMENT}` : ''),
    securityId: String(x.SECURITY_ID || x.SEM_SMST_SECURITY_ID || x.SECURITYID || ''),
    symbol: x.SYMBOL_NAME || x.SEM_TRADING_SYMBOL || x.SM_SYMBOL_NAME || '',
    tradingSymbol: x.SEM_TRADING_SYMBOL || '',
    displayName: x.DISPLAY_NAME || x.SEM_CUSTOM_SYMBOL || '',
    instrument: x.INSTRUMENT || x.SEM_INSTRUMENT_NAME || '',
    instrumentType: x.INSTRUMENT_TYPE || x.SEM_EXCH_INSTRUMENT_TYPE || '',
    underlyingSecurityId: x.UNDERLYING_SECURITY_ID || '',
    underlyingSymbol: x.UNDERLYING_SYMBOL || '',
    expiry: x.SEM_EXPIRY_DATE || '',
    strike: safeNum(x.STRIKE_PRICE || x.SEM_STRIKE_PRICE),
    optionType: x.OPTION_TYPE || '',
    lotSize: safeNum(x.LOT_SIZE || x.SEM_LOT_UNITS),
    tickSize: safeNum(x.TICK_SIZE || x.SEM_TICK_SIZE)
  })).filter(x=>x.securityId && x.exchangeSegment);
  instrumentByKey=new Map(instruments.map(x=>[keyOf(x.exchangeSegment,x.securityId),x]));
  instrumentLoadedAt=now();
  console.log(`Instrument master loaded: ${instruments.length}`);
...
app.get('/',(req,res)=>res.json({success:true,name:'Dhan Ultimate Backend',version:'1.0.0',message:'Universal market-data plane. Strategies belong in PWAs.',docs:'/api/v1'}));
...
wss.on('connection',(ws,req)=>{
  ...
  ws.send(JSON.stringify({type:'hello',service:'Dhan Ultimate Backend',version:'1.0.0',feedState}));
  ws.on('message',raw=>{
    try{
...
loadInstruments().catch(e=>console.log('Instrument master initial load failed:',e.message));
connectFeed();
